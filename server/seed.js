...
async function seedDatabase() {
  ...
  try {
    ...
    const tables = ['course_feedback', 'course_materials', 'exam_grades', 'receipts', 'registered_courses', 'courses', 'users'];
    ...
    // IDs 1-5: Admins
    for (let i = 0; i < 5; i++) {
      users.push([faker.internet.username(), faker.internet.email(), 'password123', 'admin', '1980-01-01', 'System Admin']);
    }
    // IDs 6-15: Educators
    for (let i = 0; i < 10; i++) {
      users.push([faker.person.fullName(), faker.internet.email(), 'password123', 'educator', '1985-05-15', 'Course Instructor']);
    }
    // IDs 16-115: Students
    for (let i = 0; i < 100; i++) {
      users.push([faker.person.fullName(), faker.internet.email(), 'password123', 'student', '2005-08-20', 'Active Student']);
    }

    await connection.query(`INSERT INTO users (username, email, password, role, dob, bio) VALUES ?`, [users]);
    ...
    for (let educatorId = 6; educatorId <= 15; educatorId++) {
      for (let j = 0; j < 2; j++) { 
        const price = faker.number.float({ min: 50, max: 200, fractionDigits: 2 });
        courses.push([
        ...
        courseData.push({ id: courseIdCounter, price: price, educatorId: educatorId });
        courseIdCounter++;
      }
    }
    await connection.query(`INSERT INTO courses (course_code, title, description, price, rating, instructor_id) VALUES ?`, [courses]);
    ...
    await connection.query(`INSERT INTO course_materials (course_id, uploader_id, filename, file_path) VALUES ?`, [materials]);
    ...
    for (const course of courseData) {
      const selectedStudents = faker.helpers.arrayElements(
        Array.from({ length: 100 }, (_, i) => i + 16), 12
      );
      ...
      for (const studentId of selectedStudents) {
        ...
        enrollments.push([studentId, course.id, status, date]);
        receipts.push([
          studentId, course.id, course.price, 
        ...
        if (isCompleted) {
          grades.push([
            ...
            date, course.educatorId 
          ]);
        }
      }
    }

    await connection.query(`INSERT INTO registered_courses (student_id, course_id, status, enroll_date) VALUES ?`, [enrollments]);
    await connection.query(`INSERT INTO receipts (user_id, course_id, amount, payment_method, account_number, transaction_date) VALUES ?`, [receipts]);
    await connection.query(`INSERT INTO exam_grades (student_id, course_id, grade, issued_date, graded_by) VALUES ?`, [grades]);

    // 5. GENERATE VARIED COURSE FEEDBACK (Reviews)
    ...
    for (const course of courseData) {
      ...
      const reviewers = faker.helpers.arrayElements(enrolledStudentsInCourse, faker.number.int({ min: 3, max: 6 }));
      ...
      for (const studentId of reviewers) {
        ...
        feedbacks.push([course.id, studentId, content, rating, date]);
      }
    }

    await connection.query(`INSERT INTO course_feedback (course_id, user_id, content, rating, date_time) VALUES ?`, [feedbacks]);
