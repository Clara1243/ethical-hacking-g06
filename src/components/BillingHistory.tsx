...
export const BillingHistory: React.FC<BillingHistoryProps> = ({ receipts, userEmail, onViewReceipt }) => {
    ...
    <div id="billing-history-container" className="py-8 max-w-4xl mx-auto px-4 space-y-6">
      ...
      {userReceipts.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center space-y-3">
          <div className="mx-auto w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400"><Table size={20} /></div>
          <p className="text-sm font-medium text-gray-650">No purchases found in your account.</p>
          <p className="text-xs text-gray-400 max-w-sm mx-auto">Browse the course catalog, choose a topic, and enroll to populate invoice histories.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-gray-150">
                {userReceipts.map((rec) => (
                  <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-4 px-4 font-mono font-bold text-slate-900">${rec.amount.toFixed(2)}</td>
                    <td className="py-4 px-4">
                      <div className="inline-flex items-center gap-1 text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-100 text-[10px] uppercase font-bold font-sans">
                        <CheckCircle2 size={10} className="text-emerald-500" /><span>{rec.status}</span>
                      </div>
                    </td>
                    <td className="py-4 px-4 text-right">
                      <button onClick={() => onViewReceipt(rec.id)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[10px] uppercase rounded-lg shadow-xs transition-transform transform active:scale-95 cursor-pointer">
